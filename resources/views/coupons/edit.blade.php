    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('coupons.update', $coupon->id) }}" method="POST" class="row">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" name="code" class="form-control" id="code" placeholder="كود الكوبون" value="{{ old('code', $coupon->code) }}">
                        </div>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select name="discount_type" id="discount_type" class="form-control" required>
                                ...
                                <option value="amount" {{ old('discount_type', $coupon->discount_type) == 'amount' ? 'selected' : '' }}>مبلغ ثابت</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-4" id="discount_percentage_group">
                            ...
                            <input type="number" step="0.01" name="discount_percentage" class="form-control" id="discount_percentage" placeholder="نسبة الخصم" value="{{ old('discount_percentage', $coupon->discount_percentage) }}" min="0" max="100" required>
                        </div>

                        <!-- مبلغ الخصم -->
                        <div class="form-group col-md-4" id="discount_amount_group" style="display: none;">
                            <label for="discount_amount">مبلغ الخصم</label>
                            <input type="number" step="0.01" name="discount_amount" class="form-control" id="discount_amount" placeholder="مبلغ الخصم" value="{{ old('discount_amount', $coupon->discount_amount) }}" min="0">
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="date" name="start_date" class="form-control" id="start_date" value="{{ old('start_date', $coupon->start_date->format('Y-m-d')) }}" required>
                        </div>

                        <!-- تاريخ النهاية -->
                        <div class="form-group col-md-6">
                            <label for="end_date">تاريخ النهاية</label>
                            <input type="date" name="end_date" class="form-control" id="end_date" value="{{ old('end_date', $coupon->end_date->format('Y-m-d')) }}" required>
                        </div>

                        <!-- الوصف (اختياري) -->
                        <div class="form-group col-md-12">
                            <label for="description">الوصف (اختياري)</label>
                            <textarea name="description" class="form-control" id="description" placeholder="وصف الكوبون">{{ old('description', $coupon->description) }}</textarea>
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#product_id').change(function() {
            ...
            loadVariants(productId, '{{ old('variant_id', $coupon->variant_id) }}');
        });

        // إذا كان هناك منتج محدد سابقاً (مثلاً بعد فشل التحقق من صحة النموذج)
        @if(old('product_id', $coupon->product_id))
            loadVariants('{{ old('product_id', $coupon->product_id) }}', '{{ old('variant_id', $coupon->variant_id) }}');
        @endif
    });
</script>
@endsection
